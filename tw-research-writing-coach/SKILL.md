...
## 🧪【STORM 接地鉤子】論點引用接地性驗證（可選）

De-AI 掃描完成後，在進行三版本改寫之前，提議執行論點接地性驗證：

> 「結構已審閱，De-AI 掃描完成。
>  我現在逐一掃描你的核心論點，確認每個聲明是否有文獻支撐——
>  懸空論點（無來源強主張）會標記 ❌，要執行嗎？」

若使用者同意：
→ 呼叫 **tw-research-grounded-dialogue**（stage="writing"，mode="citation-verify"）
→ 傳入：使用者提供的原始段落
→ 啟用視角：SKEPTIC（主要）+ METHOD（輔助）
→ 輸出：論點接地性驗證報告
→ 改寫時優先針對 ❌ 懸空論點與 ⚠️ 待引用論點進行強化
